...
export async function updateScore(req, res) {
    try {
        ...
        const { idSubject, diemCC, diemGK, diemThi, diemTB, idSemester } = req.body;
        await validate({ idStudent, idSubject, diemCC, diemGK, diemThi, diemTB, idSemester });

        const data = await scoreService.updateScore(idStudent, { idSubject, diemCC, diemGK, diemThi, diemTB, idSemester });
